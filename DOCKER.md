...
本指南说明如何在生产环境中用 Docker 运行 BookFree,包括两步部署、TLS、反向代理、备份、升级以及需要注意的常见问题。

整个部署流程只有两步:**写 `.env`** + **`docker compose up -d --build`**。
...
- **空闲 RSS 约 32 MB,容器硬上限 256 MB。** compose 文件固定了内存限制,避免失控的 worker 把宿主机打到 OOM。
- **持久化卷。** 主机上的 `./data/` 保存 SQLite 数据库和所有上传书籍;绑定挂载而不是命名卷,方便 `tar` 备份。
...
`.env` 与 `docker-compose.yml` 同目录;Compose v2 会自动加载,不需要 `--env-file`。
...
`.env` 至少要有 `BOOKFREE_APP_SECRET` 这一行。其它字段(注册开关、AI Key、时区等)可参考 `.env.example` 也写到 `.env`,不写就用 `docker-compose.yml` 里的默认值。
...
第一次构建约 2 分钟,完成后访问 <http://127.0.0.1:8788>。

如果忘了写 `.env` 里的 `BOOKFREE_APP_SECRET`,Compose 会立即拒绝启动并打印中文提示告诉你去 `.env` 填,而不是神秘地起一个无密钥的容器。
...
**方式 B — 单用户私人部署。** 在 `.env` 保留 `BOOKFREE_ALLOW_REGISTRATION=1`。
...
| `GOMEMLIMIT` | `240MiB` | 软堆目标。越低 GC 越频繁。 |
| `GOGC` | (Go 默认 100) | 堆增长百分比;越低 GC 越频繁。 |
| `TZ` | `Asia/Shanghai` | 调整为你的本地时区 |
| `ANTHROPIC_API_KEY` | 空 | 启用阅读器右侧 AI 对话面板 |
...
- 未在 `.env` 中设置 `BOOKFREE_APP_SECRET` — compose 会打印明确的错误信息,按提示去 `.env` 填一行 `BOOKFREE_APP_SECRET=$(openssl rand -hex 32)`
